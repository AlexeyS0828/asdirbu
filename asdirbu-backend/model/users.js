...
class Users
{
	...
	async getUserReview(user_id){
		try{
			return await this.db.query('SELECT urat.*,u.name,u.email,u.profile_image FROM user_ratings as urat LEFT JOIN users u ON u.id = urat.given_by_user_id WHERE urat.user_id = ? ORDER BY urat.id DESC',[user_id]);
		}catch(exx){
			console.log(exx);
			return false;
		}
	...
	async storeUserReview(bookingId,user_id,givenById,rating,review){
		try{
			return await this.db.query('INSERT INTO user_ratings(booking_id,user_id,given_by_user_id,ratings,review) values(?,?,?,?,?)',[bookingId,user_id,givenById,rating,review]);
		}catch(exx){
			console.log(exx);
			return false;
		}
	...
	async getAllBookingJobWise(previousDate){
		try {
			...
			return await this.db.query('SELECT bookings.booked_user_id as userId,bookings.id As bookingId,users.email as email,frelancer.email as frelancerEmail FROM bookings  INNER JOIN greitas_skelbimas ON greitas_skelbimas.id  = bookings.jobId AND paslaugos_priėmimo_vieta = ? LEFT JOIN users ON users.id = bookings.user_id LEFT JOIN users AS frelancer ON frelancer.id = bookings.booked_user_id WHERE jobId!=0',[previousDate]);
		}
		catch(error){
			console.log(error);
			return false;
	...
	async getBookingByDayWise(){
		try{
			...
			return await this.db.query('SELECT users.email AS email,bookings.booked_user_id AS userId,bookings.id AS bookingId,hire_day.dates AS hireList,frelancer.email as frelancerEmail FROM bookings INNER JOIN hire_day ON hire_day.id = bookings.hire_id LEFT JOIN users ON users.id = bookings.user_id LEFT JOIN users as frelancer ON frelancer.id = bookings.booked_user_id  WHERE hire_id !=0');
		}
		catch(error)
		{
			console.log(error);
	...
	async addUserChat(fromUserId,toUserId,postJobId,status=1){
		try {
			return await this.db.query("INSERT INTO nearest_user(postjob_id,from_user_id,to_user_id,status) VALUES(?,?,?,?)",[postJobId,fromUserId,toUserId,status]);
		}
		catch(error) {
			console.log(error);
			return false;
	...
	async changeUserChatStatus(id,status){
		try{
			return await this.db.query("UPDATE nearest_user SET status = ? WHERE id = ? ",[status,id]);
		}
		catch(error){			
			console.log(error);
			return false;
	...
	async updateProfile(name,lastname,email,phone,companyName,companyCode,dob,hourrate,type,serviceName,specialty,backAccountNumber,service,aboutMe,address,city,houseNumber,profileImage,images,id){
		try {	
			if(images!=""){
				return await this.db.query(`UPDATE users SET name = ?,last_name = ?,email = ?,phone = ?,dob = ?,hourrate = ?,type = ?,serviceName = ?,specialty = ?,bank_ac_no = ?,service = ?,about_me = ?,address = ?,city = ?,houseNumber = ?,profile_image = ?,image = ? WHERE id = ?`,[name,lastname,email,phone,dob,hourrate,type,serviceName,specialty,backAccountNumber,service,aboutMe,address,city,houseNumber,profileImage,images,id]);
			}
			else{
				return await this.db.query(`UPDATE users SET name = ?,last_name = ?,email = ?,phone = ?,dob = ?,hourrate = ?,type = ?,serviceName = ?,specialty = ?,bank_ac_no = ?,service = ?,about_me = ?,address = ?,city = ?,houseNumber = ?,profile_image = ? WHERE id = ?`,[name,lastname,email,phone,dob,hourrate,type,serviceName,specialty,backAccountNumber,service,aboutMe,address,city,houseNumber,profileImage,id]);
			}
		} 		
		catch (error) {
			console.log(error);
	...
	async addNewUser(name,lastname,email,phone,companyName,companyCode,dob,hourrate,type,serviceName,specialty,backAccountNumber,service,aboutMe,address,city,houseNumber,profileImage,images,password,userType){
		try {	
			
			return await this.db.query(`INSERT INTO users(name,last_name,email,phone,dob,hourrate,type,serviceName,specialty,bank_ac_no,service,about_me,address,city,houseNumber,profile_image,image,password,userType,is_user_verified,isApproved) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,[name,lastname,email,phone,dob,hourrate,type,serviceName,specialty,backAccountNumber,service,aboutMe,address,city,houseNumber,profileImage,images,password,userType,1,1]);
		} 		
		catch (error) {
			console.log(error);
			return false;
	...
	async getTimeSheetList(datelist,userId){
		try {
			return await this.db.query("SELECT start,end,date FROM user_timesheet WHERE date IN(?) AND user_id = ? ORDER BY DATE,start ASC",[datelist,userId]);
		}	
		catch (error) {
			console.log(error);
			return false;
	...
	async deleteTimeSheetDateWise(datelist,userId){
		try {
		return await this.db.query("DELETE FROM user_timesheet WHERE date IN(?) AND user_id = ?",[datelist,userId]);
		}	
		catch (error) {
			console.log(error);
			return false;
	...
	async getJobReviewById(bookingId){
		try {
			return await this.db.query("SELECT users.name,users.image,user_ratings.ratings,user_ratings.review FROM bookings INNER JOIN user_ratings ON booking_id= bookings.id INNER JOIN users ON users.id =  bookings.booked_user_id WHERE bookings.id = ?",[bookingId]);
		}	
		catch (error) {
			console.log(error);
			return false;
