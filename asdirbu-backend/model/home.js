...
async getAllLocation(curruntUserId){
	...
	async getSearchUsersCount(city,serviceName,otherServiceName,specialty,otherSpecialty,curruntUserId){
		try {
			return await this.db.query(`SELECT count(id) AS userCount FROM users WHERE (IF(? = 'Kitas',(otherSpecialty=?),(specialty=?)) OR city LIKE ?) AND id != ? AND isApproved = 1 AND hourrate > 0`,[specialty,otherSpecialty,specialty,city,curruntUserId]);
		}
		catch(error) {
	...
	async getSearchJobCount(city,serviceName,otherServiceName,specialty,otherSpecialty,curruntUserId){
		try {
			return await this.db.query(`SELECT COUNT(job.id) AS jobCount FROM greitas_skelbimas AS job LEFT JOIN users ON users.id = job.user_id  WHERE job.skelbimo_pavadinimas !="" AND job.skelbimo_pavadinimas != "undefined" AND job.user_id != ? AND (IF(? = 'Kitas',(job.otherSpecialty=?),(job.specialty=?)) OR paslaugos_suteikimo_vieta LIKE ?)`,[curruntUserId,specialty,otherSpecialty,specialty,city]);
		}
		catch(error) {
			console.log("db error message ",error);
			return false;
	...
	async getSearchNewUsers(city,serviceName,otherServiceName,specialty,otherSpecialty,limit,offset,curruntUserId){
		try {
			return await this.db.query(`SELECT u.*,(SELECT AVG(user_ratings.ratings) FROM user_ratings WHERE user_ratings.user_id = u.id) AS user_rating FROM users  AS u WHERE (IF(? = 'Kitas',(otherSpecialty=?),(specialty=?)) OR city LIKE ?) AND u.id != ? AND isApproved = 1 AND u.hourrate > 0 ORDER BY u.id  DESC LIMIT ? OFFSET ?`,[specialty,otherSpecialty,specialty,city,curruntUserId,limit,offset]);
		}catch(error){
			console.log("db error message ",error);
			return false;
		}
	...
	async getSearchjob(city,serviceName,otherServiceName,specialty,otherSpecialty,limit,offset,curruntUserId){
		try{
			return await this.db.query(`SELECT job.id AS id,job.pridėk_nuotrauką AS documentImage,job.skelbimo_pavadinimas AS name,IF(job.serviceName IS NULL,'',job.serviceName) AS serviceName,IF(job.otherServiceName IS NULL,'',job.otherServiceName) AS otherServiceName,job.paslaugos_kaina AS hourrate,job.latitude AS latitude,job.longitude AS longitude,users.profile_image AS profile_image,IF(users.taxPay IS NULL,0,users.taxPay) AS taxPay FROM greitas_skelbimas AS job LEFT JOIN users ON users.id = job.user_id WHERE job.skelbimo_pavadinimas !="" AND job.skelbimo_pavadinimas != "undefined" AND job.user_id != ? AND (IF(? = 'Kitas',(job.otherSpecialty=?),(job.specialty=?)) OR paslaugos_suteikimo_vieta LIKE ?) ORDER BY job.id DESC limit ? offset ?`,[curruntUserId,specialty,otherSpecialty,specialty,city,limit,offset]);
		}catch(error){
			console.log("db error message ",error);
			return false;
		}
	...
	async getFilterNewUsers(price,distance,limit,offset,curruntUserId,latitude,longitude){
		try{
			let query = "";
			if(price > 0 && distance > 0 && latitude != 0 && longitude != 0){
			    query = `SELECT u.*,(SELECT AVG(user_ratings.ratings) FROM user_ratings WHERE user_ratings.user_id = u.id) AS user_rating, (111.111 * DEGREES(ACOS(LEAST(1.0, COS(RADIANS(${latitude})) * COS(RADIANS(u.Latitude)) * COS(RADIANS(${longitude} - u.Longitude)) + SIN(RADIANS(${latitude})) * SIN(RADIANS(u.Latitude)))))) AS distance_km FROM users AS  u 
			   ...
			   query = `SELECT u.*,(SELECT AVG(user_ratings.ratings) FROM user_ratings WHERE user_ratings.user_id = u.id) AS user_rating FROM users AS  u 
