...
class Postjob
{
	...
	async createjob(user_id,skelbimo_pavadinimas,papasakok_plačiau,Raktažodžiai,paslaugos_kaina,paslaugos_suteikimo_vieta,distance,pridėk_nuotrauką,paslaugos_priėmimo_vieta,job_type,paslaugos_priemimo_laikas,latitude,longitude,specialty,otherSpecialty,placeGoodsDelivery){
		try{	
			return await this.db.query(`INSERT INTO greitas_skelbimas(user_id,skelbimo_pavadinimas,papasakok_plačiau,Raktažodžiai,paslaugos_kaina,paslaugos_suteikimo_vieta,distance,pridėk_nuotrauką,paslaugos_priėmimo_vieta,job_type,paslaugos_priemimo_laikas,latitude,longitude,specialty,otherSpecialty,placeGoodsDelivery) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [user_id,skelbimo_pavadinimas,papasakok_plačiau,Raktažodžiai,paslaugos_kaina,paslaugos_suteikimo_vieta,distance,pridėk_nuotrauką,paslaugos_priėmimo_vieta,job_type,paslaugos_priemimo_laikas,latitude,longitude,specialty,otherSpecialty,placeGoodsDelivery]);
		} 		
		catch (error) 
		{
			console.log(error);
	...
	async getUserListBylatlong(distance,currentLatitude,currentLongitude,job_type,specialty,otherSpecialty) {
		try {
			if(job_type==2){
				...
				return await this.db.query(`SELECT id,email from users as b WHERE   (111.111 * DEGREES(ACOS(LEAST(1.0, COS(RADIANS(?)) * COS(RADIANS(b.Latitude)) * COS(RADIANS(? - b.Longitude)) + SIN(RADIANS(?)) * SIN(RADIANS(b.Latitude)))))) < ? AND  IF(? = 'Kitas',(otherSpecialty=?),(specialty=?))`,[currentLatitude,currentLongitude,currentLatitude,distance,specialty,otherSpecialty,specialty]);
			}
		}
		catch(err) {
			return false;
