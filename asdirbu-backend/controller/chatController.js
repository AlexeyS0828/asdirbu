...
chatController.getUserChat = function(req,res){
	...
	Chat.readMessageById(req.id,req.body.user_id).then();
...
chatController.countMessage = async function(req,res) {
	let data = await Chat.countMessage(req.id);
	return res.status(200).send({success:true,message:"Sėkmingai gaukite pranešimų skaičių",data});	
}
